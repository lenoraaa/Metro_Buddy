...
export async function analyzeImage(imageBase64: string, prompt: string): Promise<string> {
    ...
    try {
        // Use gemini-flash-lite-latest for high quota and stable performance
        ...
        return `Sorry, I couldn't analyze that image. Error: ${errorMessage}`;
    }
}
...
export async function parseIntent(text: string): Promise<{ start?: string; destination?: string } | null> {
    ...
    try {
        // Use gemini-flash-lite-latest for consistency and higher quota
        const model = genAI.getGenerativeModel({ model: "gemini-flash-lite-latest" });
